...
-(void) imageRepresentation {
    ...
    UIImage *img = [self cutImage:image];//截取
    
    //[self.delegate getSignatureImg:[self scaleToSize:img]];
    self.SignatureImg = [self scaleToSize:img];//压缩
}
#pragma mark -压缩图片
...
- (UIImage *)scaleToSize:(UIImage *)img {
    ...
    UIGraphicsBeginImageContext(size); //根据给定得图片，从其指定区域截取一张新得图片
    [img drawInRect:rect];
...
- (UIImage *)cutImage:(UIImage *)image
{
    ...
    CGImageRef imageRef = CGImageCreateWithImageInRect([image CGImage], rect);//裁图
    UIImage * img = [UIImage imageWithCGImage:imageRef];
    
    UIImage *lastImage = [self addText:img text:self.showMessage];
    CGImageRelease(imageRef);//imagePartRef生成新图片之后就没用了，需要释放掉
    [self setNeedsDisplay];
    return lastImage;
}
...
- (void)pan:(UIPanGestureRecognizer *)pan {
    ...
    CGPoint midPoint = midpoint(previousPoint, currentPoint); //计算2点中点
    ...
    if (pan.state ==UIGestureRecognizerStateBegan) {
        [path moveToPoint:currentPoint];//设置初始线段的起点
